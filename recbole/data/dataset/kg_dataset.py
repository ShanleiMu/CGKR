...
class KnowledgeBasedDataset(Dataset):
    ...
    def _create_ckg_nx_graph(self, form='nx', show_relation=False):
        user_num = self.user_num
        ...
        src = torch.cat([user, item, head_entity, tail_entity])
        tgt = torch.cat([item, user, tail_entity, head_entity])

        if show_relation:
            ui_rel_num = user.shape[0]
            ui_rel_id, iu_rel_id = 0, 1
            kg_rel = kg_tensor[self.relation_field] + 2
            ui_rel = torch.full((ui_rel_num,), ui_rel_id, dtype=kg_rel.dtype)
            iu_rel = torch.full((ui_rel_num,), iu_rel_id, dtype=kg_rel.dtype)
            edge = torch.cat([ui_rel, iu_rel, kg_rel, kg_rel + (self.relation_num - 2)])

        if form == 'nx':
            ckg_graph = nx.MultiDiGraph()
            src, tgt, edge = src.numpy(), tgt.numpy(), edge.numpy()
            for h_id, t_id, r_id in zip(src, tgt, edge):
    ...
    def ckg_graph(self, form='coo', value_field=None):
        ...
        if form in ['coo', 'csr']:
            ...
            return self._create_ckg_nx_graph(form, show_relation)
        else:
            raise NotImplementedError('ckg graph format [{}] has not been implemented.')

    @dlapi.set()
    def kg_neighbors(self, max_neighbor_size=32,
                     relations=False, random=False, repeat=False, relation_sets=None):
        neighbors, neighbor_relations, neighbor_length = [], [], []
        entity2neighbors = collections.defaultdict(list)
        entity2relations = collections.defaultdict(list)
        for head, relation, tail in zip(self.head_entities, self.relations,
                                        self.tail_entities):
            if relation_sets is None or relation in relation_sets:
                entity2neighbors[head].append(tail)
                entity2neighbors[tail].append(head)
                entity2relations[head].append(relation)
                entity2relations[tail].append(relation)
        for i in range(self.entity_num):
            if i in entity2neighbors:
                if len(entity2neighbors[i]) > max_neighbor_size:
                    if random:
                        indices = np.random.choice(len(entity2neighbors[i]), size=max_neighbor_size, replace=False)
                        neighbors.append(np.array(entity2neighbors[i])[indices])
                        neighbor_relations.append(np.array(entity2relations[i])[indices])
                    else:
                        neighbors.append(entity2neighbors[i][:max_neighbor_size])
                        neighbor_relations.append(entity2relations[i][:max_neighbor_size])
                    neighbor_length.append(max_neighbor_size)
                else:
                    if repeat:
                        indices = np.random.choice(len(entity2neighbors[i]),
                                                   size=max_neighbor_size - len(entity2neighbors[i]), replace=True)
                        indices = np.hstack((range(len(entity2neighbors[i])), indices))
                        neighbors.append(np.array(entity2neighbors[i])[indices])
                        neighbor_relations.append(np.array(entity2relations[i])[indices])
                        neighbor_length.append(max_neighbor_size)
                    else:
                        neighbors.append(entity2neighbors[i] + [0] *
                                         (max_neighbor_size - len(entity2neighbors[i])))
                        neighbor_relations.append(entity2relations[i] + [0] * (
                                max_neighbor_size - len(entity2neighbors[i])))
                        neighbor_length.append(len(entity2neighbors[i]))
            else:
                neighbors.append([0] * max_neighbor_size)
                neighbor_relations.append([0] * max_neighbor_size)
                neighbor_length.append(1)
        if relations:
            return np.array(neighbors), np.array(neighbor_relations), \
                   np.array(neighbor_length)
        else:
            return np.array(neighbors), np.array(neighbor_length)

    @dlapi.set()
    def ips_kg_neighbors(self, max_neighbor_size=32):
        neighbors = []
        entity2neighbors = collections.defaultdict(list)
        entity2count = collections.defaultdict(int)
        for head, tail in zip(self.head_entities, self.tail_entities):
            entity2neighbors[head].append(tail)
            entity2neighbors[tail].append(head)
            entity2count[head] += 1
            entity2count[tail] += 1
        ...
        for entity in entity2count:
            entity2ips[entity] = 1 / entity2count[entity]

        for i in range(self.entity_num):
            if i in entity2neighbors:
                temp_neighbors = entity2neighbors[i]
                ips_list = [entity2ips[neigh] for neigh in temp_neighbors]
                sum_ips = sum(ips_list)
                probs = [ips / sum_ips for ips in ips_list]
                neighbors.append(np.random.choice(temp_neighbors, size=max_neighbor_size, replace=True, p=probs))
            else:
                neighbors.append([0] * max_neighbor_size)
        return np.array(neighbors)

    @dlapi.set()
    def user2neighbors(self, max_neighbor_size=32):
        user2items = collections.defaultdict(list)
        inter_tensor = self._dataframe_to_interaction(self.inter_feat)
        users = inter_tensor[self.uid_field].numpy()
        items = inter_tensor[self.iid_field].numpy()
        for user, item in zip(users, items):
            user2items[user].append(item)

        user2neighbors = []
        for u in range(self.user_num):
            if len(user2items[u]) > 0:
                if len(user2items[u]) > max_neighbor_size:
                    indices = np.random.choice(len(user2items[u]), size=max_neighbor_size, replace=False)
                    user2neighbors.append(np.array(user2items[u])[indices])
                else:
                    user2neighbors.append(user2items[u] + [0] * (max_neighbor_size - len(user2items[u])))
            else:
                user2neighbors.append([0] * max_neighbor_size)
        return np.array(user2neighbors)
    ...
    def history_records(self):
        ...
        user_num = self.user_num
        true_num = max([len(user2items[user]) for user in user2items])
        history_records = torch.zeros(user_num, true_num)

        for user in user2items.keys():
            true_list = user2items[user]
            true_list += [-1] * (true_num - len(true_list))
            history_records[user] = torch.tensor(true_list, dtype=torch.long)
        return history_records

    @dlapi.set()
    def relation2candidates(self):
        relation2candidates = collections.defaultdict(set)
        for relation, tail in zip(self.relations, self.tail_entities):
            relation2candidates[relation].add(tail)
        for relation in relation2candidates:
            relation2candidates[relation] = list(relation2candidates[relation])
        return relation2candidates

    @dlapi.set()
    def user2interactions(self):
        user2interactions = collections.defaultdict(set)
        inter_tensor = self._dataframe_to_interaction(self.inter_feat)
        users = inter_tensor[self.uid_field].numpy()
        items = inter_tensor[self.iid_field].numpy()
        for user, item in zip(users, items):
            user2interactions[user].add(item)
        return user2interactions
