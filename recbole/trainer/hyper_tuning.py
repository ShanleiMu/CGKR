...
class HyperTuning(object):
    ...
    def __init__(self, objective_function, space=None, params_file=None, fixed_config_file_list=None,
                 algo='exhaustive', max_evals=100, already_paras=None, result_file=None):
        ...
        self.already_paras = already_paras
    ...
    def params2filename(params):
        filename = ''
        for param_name in params:
            filename += param_name + '_' + str(params[param_name]) + '-'
        return filename[:-1] + '.pth'
    ...
    def write_result_to_file(self, params_str):
        with open(self.result_file, 'a') as fp:
            fp.write(params_str + '\n')
            fp.write('Valid result:\n' + dict2str(
                self.params2result[params_str]['best_valid_result']) + '\n')
            fp.write('Test result:\n' + dict2str(
                self.params2result[params_str]['test_result']) + '\n\n')
    ...
    def trial(self, params):
        ...
        config_dict['checkpoint_file'] = self.params2filename(params)
        if params_str in self.already_paras:
            return {'loss': 0, 'status': hyperopt.STATUS_OK}
        result_dict = self.objective_function(config_dict, self.fixed_config_file_list)
        self.params2result[params_str] = result_dict
        if self.result_file:
            self.write_result_to_file(params_str)
