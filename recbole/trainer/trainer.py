...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        if 'noise' in config['dataset']:
            self.num = dataset2num[config['dataset'].split('_')[0]]
            print(self.num)
        else:
            self.num = None
        ...
        if config['checkpoint_file']:
            saved_model_file = self.config['model'] + '-' + self.config['dataset'] \
                               + '-' + self.config['checkpoint_file']
        else:
            saved_model_file = '{}-{}-{}.pth'.format(self.config['model'],
                                                     self.config['dataset'], get_local_time())
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        if self.num:
            temp_pos_idx, new_pos_len_list = [], []
            begin = 0
            for idx, pos_len in enumerate(pos_len_list):
                temp = []
                for v in pos_idx[begin:begin + pos_len].numpy():
                    threshold = self.num + self.tot_item_num * idx
                    if v < threshold:
                        temp.append(v)
                new_pos_len_list.append(len(temp))
                temp_pos_idx += temp
                begin += pos_len
            pos_len_list = new_pos_len_list
            pos_idx = torch.from_numpy(np.array(temp_pos_idx))
