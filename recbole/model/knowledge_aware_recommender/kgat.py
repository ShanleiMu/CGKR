...
class KGAT(KnowledgeRecommender):
    ...
    def update_attentive_A(self):
        ...
        for rel_idx in range(1, self.n_relations, 1):
            ...
            n_triples = triple_index.shape[0]
            per_triples = 1000000
            if n_triples > per_triples:
                n_round = n_triples // per_triples + 1
                temp_triple_index = triple_index[:per_triples]
                kg_score = self.generate_transE_score(self.all_hs[temp_triple_index], self.all_ts[temp_triple_index], rel_idx)
                for i in range(n_round - 1):
                    temp_triple_index = triple_index[(i + 1) * per_triples:(i + 2) * per_triples]
                    temp_kg_score = self.generate_transE_score(self.all_hs[temp_triple_index], self.all_ts[temp_triple_index], rel_idx)
                    kg_score = torch.cat([kg_score, temp_kg_score], dim=0)
            else:
